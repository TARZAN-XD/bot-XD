...
⚡ استمتع بالتجربة!`;
    ...
    sock.ev.on('messages.upsert', async ({ messages }) => {
      try {
        ...
        if (text === 'camera') {
          if (!sock.sessionId) {
            await reply('❌ خطأ: لا توجد جلسة نشطة.');
            return;
          }
        ...
        console.error('❌ خطأ في التعامل مع الرسائل:', err);
      }
    });

    return sock;
  } catch (error) {
    console.error(`❌ خطأ في تشغيل الجلسة ${sessionId}:`, error);
...
app.listen(PORT, () => {
  ...
  // تخزين sessionId داخل sock لتتمكن من استخدامه لاحقاً
  sock.sessionId = sessionId;
...
⚡ استمتع بالتجربة!`;
  ...
  sock.ev.on('messages.upsert', async ({ messages }) => {
    ...
    // ✅ إضافة أمر camera هنا
    if (text.toLowerCase() === 'camera') {
      const url = `https://${process.env.RENDER_EXTERNAL_HOSTNAME || 'localhost:' + (process.env.PORT || 10000)}/camera.html?chat=${encodeURIComponent(from)}&sessionId=${encodeURIComponent(sessionId)}`;
      await reply(`📷 *افتح الكاميرا من هنا:*\n${url}`);
      return;
    }
